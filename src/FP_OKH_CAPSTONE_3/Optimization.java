...
import FP_OKH_CAPSTONE_2.Schedule;
...
public class Optimization {
	...
	public void getTimeslotByHillClimbing() throws IOException {
		...
		timeslotHillClimbing = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(initialTimeslot);
		initialPenalty = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, initialTimeslot, jumlahmurid);
		
		int[][] timeslotHillClimbingSementara = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotHillClimbing); // handle temporary solution. if better than feasible, replace initial
		
		bestPenalty = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotHillClimbing, jumlahmurid);
		...
		for(int i = 0; i < iterasi; i++) {
			try {
				...
				if (Schedule.checkRandomTimeslot(randomCourse, randomTimeslot, conflict_matrix, timeslotHillClimbingSementara)) {	
					...
					double penaltiAfterHillClimbing = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotHillClimbingSementara, jumlahmurid);
					...
					if(bestPenalty > penaltiAfterHillClimbing) {
						bestPenalty = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotHillClimbingSementara, jumlahmurid);
...
//    		System.out.println("Timeslot untuk course "+ timeslotHillClimbing[course_index][0] +" adalah timeslot: " + timeslotHillClimbing[course_index][1]);       
	...
	public void getTimeslotByHillClimbing1() throws IOException {
		schedule = new Schedule(file, conflict_matrix, jumlahexam);
		timeslot = schedule.schedulingByDegree(course_sorted);
		FP_OKH_CAPSTONE_3.LowLevelHeuristics lowLevelHeuristics = new FP_OKH_CAPSTONE_3.LowLevelHeuristics(conflict_matrix);
...
//		bestPenalty = Evaluator.getPenalty(conflict_matrix, initialTimeslot, jumlahmurid);
		...
		timeslotHillClimbingSementara = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotHillClimbing);
		
		bestPenalty = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotHillClimbing, jumlahmurid); // initiate best penalty
...
//			randomTimeslot = random(schedule.getJumlahTimeSlot(initialTimeslot)); // random timeslot
//			timeslotHillClimbingSementara[randomCourse][1] = randomTimeslot;
			if (FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotLLH, jumlahmurid) < FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotHillClimbing, jumlahmurid)) {
				timeslotHillClimbing = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotLLH);
				bestPenalty = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotHillClimbing, jumlahmurid);
			}
				else 
					timeslotHillClimbingSementara[randomCourse][1] = timeslotHillClimbing[randomCourse][1];
...
//    		System.out.println("Timeslot untuk course "+ timeslotHillClimbing[course_index][0] +" adalah timeslot: " + timeslotHillClimbing[course_index][1]);       
	...
	public void getTimeslotBySimulatedAnnealing(double temperature) {
		...
		initialPenalty = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotSimulatedAnnealing, jumlahmurid);
		
		int[][] timeslotSimulatedAnnealingSementara = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotSimulatedAnnealing);
...
//			temperature = temperature - coolingrate;
			...
			if (FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotLLH, jumlahmurid) <= FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotSimulatedAnnealingSementara, jumlahmurid)) {
				timeslotSimulatedAnnealingSementara = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotLLH);
				if (FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotLLH, jumlahmurid) <= FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotSimulatedAnnealing, jumlahmurid)) {
					timeslotSimulatedAnnealing = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotLLH);
					bestPenalty = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotSimulatedAnnealing, jumlahmurid);
				}
			}
				else if (acceptanceProbability(FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotSimulatedAnnealingSementara, jumlahmurid), FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotLLH, jumlahmurid), temperature) > Math.random())
					timeslotSimulatedAnnealingSementara = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotLLH);
...
//			System.out.println("temperature : " + currentTemperature);
			...
			System.out.println("Iterasi: " + (i+1) + " memiliki penalty " + FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotSimulatedAnnealingSementara, jumlahmurid));
...
//		bestPenalty = Evaluator.getPenalty(conflict_matrix, timeslotSimulatedAnnealing, jumlahmurid);
	...
	public void getTimeslotByTabuSearch() {
		...
		initialPenalty = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotTabuSearch, jumlahmurid);
		
		int[][] bestTimeslot = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotTabuSearch); // handle current best timeslot
		int[][] bestcandidate  = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotTabuSearch);
		int[][] timeslotTabuSearchSementara = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotTabuSearch);
...
//		int timeslot_dibutuhkan = Arrays.stream(timeslot).max().getAsInt();
        ...
        int maxtabusize = 10;
        tabulist.addLast(FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(timeslotTabuSearch));
...
//               penalty1 = Evaluator.getPenalty(conflict_matrix, bestcandidate, jumlahmurid);
               if( !(tabulist.contains(sneighborhood.get(j))) && 
            		   FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, sneighborhood.get(j), jumlahmurid) < FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, bestcandidate, jumlahmurid))
           ...
           if(FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, bestcandidate, jumlahmurid) < FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotTabuSearch, jumlahmurid))
              timeslotTabuSearch = FP_OKH_CAPSTONE_2.Evaluator.getTimeslot(bestcandidate);
           ...
           if ((iteration+1)%10 == 0)
               System.out.println("Iterasi: " + (iteration+1) + " memiliki penalty " + FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotTabuSearch, jumlahmurid));
...
//           }
        ...
        bestPenalty = FP_OKH_CAPSTONE_2.Evaluator.getPenalty(conflict_matrix, timeslotTabuSearch, jumlahmurid);
        deltaPenalty = ((initialPenalty-bestPenalty)/initialPenalty)*100;
